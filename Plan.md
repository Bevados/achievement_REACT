...
5. `package.json` получил `engines.node = 24.x` для Vercel runtime.
...
## Прогресс по мелким UX-правкам после CRUD

1. На карточке коллекции в private-списке появились маленькие inline-кнопки `Редактировать` и `Удалить`, чтобы не заходить в detail-страницу ради базовых CRUD-действий.
2. В шапке для авторизованного пользователя убрана дублирующая ссылка `Мои коллекции`, потому что на текущем этапе у private-навигации нет альтернативных разделов.
3. На `CollectionsPage` убраны технические/служебные пояснения, не несущие пользовательской ценности.
4. В `CollectionForm` select категории теперь стартует пустым placeholder `Например, Путешествия`, а `Своя категория` вынесена отдельным первым блоком над основным алфавитным списком preset-категорий.
5. Detail URL коллекций переведены на читаемый формат `id + slug` без отказа от `id` как основного идентификатора.
6. Синхронизация query-параметров списков и detail-фильтров больше не использует `replace`, чтобы browser history ощущалась ближе к обычной вкладке.
7. На карточке коллекции убран текст `Подробнее`, добавлены дата создания и дата обновления.
8. На private/public detail-страницах коллекций теперь показываются и `createdAt`, и `updatedAt`, а также добавлены верхние и нижние кнопки возврата к списку.
9. Панель фильтров карточек теперь раскрывается и скрывается плавно.
