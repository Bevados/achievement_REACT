...
4. `FIREBASE_PROJECT_ID`, `FIREBASE_CLIENT_EMAIL`, `FIREBASE_PRIVATE_KEY` - учетные данные сервис-аккаунта.
...
4. Явный вызов `ensureServerEnvLoaded()` делает локальный `vercel dev` ближе по поведению к seed-скриптам и уменьшает риск пустого `process.env`.
