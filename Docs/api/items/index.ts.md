...
2. Контракт входа:
3. `req` - объект запроса с потенциальным полем `userId` после `verifyAuth`.
4. `res` - объект ответа Vercel.
5. Контракт выхода:
6. Делегирует ответ в контроллеры или возвращает `405` для неподдерживаемых методов.
7. Инварианты:
8. Перед вызовом любого контроллера выполняется `verifyAuth`.
9. Если в `catch` уже были отправлены заголовки (`res.headersSent`), повторная отправка ответа не выполняется.
