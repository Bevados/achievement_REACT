...
2. `useEffect` не даёт скрытому старому значению `customCategory` остаться в форме, если пользователь вернулся на preset-категорию.
3. Форма работает с `noValidate`, чтобы ошибки показывал именно RHF/Zod-слой.
4. Кнопка `Сохранить` больше не disabled по умолчанию: форма уже работает как реальный submit-entrypoint для create/update коллекции.
5. Если create/update запрос вернул ошибку, `submitError` показывается внутри формы отдельным alert-блоком.
