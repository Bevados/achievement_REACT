...
5.1. Обработчик Escape закрывает мобильное меню и профиль.
5.2. Клик вне `menuRef` закрывает мобильное меню.
5.3. Клик вне `profileRef` закрывает профиль.
6. Анимация дропдауна профиля разделена на два состояния:
6.1. `isProfileDropdown` отвечает за mount/unmount блока.
6.2. `profileAnimateIn` отвечает за классы opacity/scale для плавного входа/выхода.
6.3. При закрытии сначала выключается анимация, затем через timeout компонент размонтируется.
7. Доступность и UX:
7.1. При открытии мобильного меню фокус переводится на первый пункт (`firstMenuItemRef`).
7.2. Для загрузочных состояний используются `aria-live` и `aria-label`, чтобы экранные читалки получали корректный контекст.
